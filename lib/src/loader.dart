...
class Loader {
  ...
  static DynamicLibrary _load() {
    ...
    //choose sys
    if (Platform.isIOS) {
      var libs = Directory.current.listSync().where((_) => _.path.endsWith('.dylib')).toList();

      print(libs);

      if (Platform.environment.containsKey(ENV_LIBRDKAFKA_ROOT)) {
        libs.addAll(Directory(Platform.environment[ENV_LIBRDKAFKA_ROOT]!).listSync().where((_) => _.path.endsWith('.dylib')).toList());
      }
